...
class MW_Validation_Rule_Required extends MW_Validation_Rule {
	...
	public function rule( MW_WP_Form_Data $Data, $key, $options = array() ) {
		...
		if ( !is_null( $value ) || is_null( $value ) && !$Data->getValues() ) {
			return;
		}
		$defaults = array(
	...
	public function admin( $key, $value ) {
		?>
		<label><input type="checkbox" <?php checked( $value[$this->name], 1 ); ?> name="<?php echo MWF_Config::NAME; ?>[validation][<?php echo $key; ?>][<?php echo esc_attr( $this->name ); ?>]" value="1" /><?php esc_html_e( 'No empty( with checkbox )', MWF_Config::DOMAIN ); ?></label>
		<?php
	}
}